...
created: 2026-05-11
updated: 2026-05-11
source: 10_Sources/roadmaps/javascript/content/@RonBj1htt6jnBt3W7zoTA.md
---
...
`Object.is()` is also not equivalent to the `===` operator. The only difference between `Object.is()` and `===` is in their treatment of signed zeros and `NaN` values. The `===` operator (and the `==` operator) treats the number values `-0` and `+0` as equal but treats `NaN` as not equal to each oth
er.
