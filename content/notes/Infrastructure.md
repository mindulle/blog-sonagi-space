...
- **GitHub Self-Hosted Runner**: K3s 클러스터 내부(`bi` 네임스페이스)에 Pod 형태로 상주.
  - **접근 권한**: Tailscale VPN이나 외부 포트 포워딩 없이, 클러스터 내부 네트워크를 통해 `llmops-instance` (100.91.9.83)의 Postgres DB(`postgres-svc`) 등에 다이렉트로 접근 가능.
  - **역할**: `llm-wiki`의 메타베이스 동기화 등 내부망 자원이 필요한 GitHub Actions 워크플로우를 담당 (`runs-on: [self-hosted, internal-db]`).
